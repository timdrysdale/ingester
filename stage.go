...
func (g *Ingester) handleTXT(path string, logger *zerolog.Logger) {
	...
	moved, err := g.MoveIfNewerThanDestinationInDir(path, g.TempTXT())
...
func (g *Ingester) handleIngestCSV(path string, logger *zerolog.Logger) {
	...
	if strings.ToLower(filepath.Base(path)) == "identity.csv" {
		...
		moved, err := g.MoveIfNewerThanDestinationInDir(path, g.Identity())
...
func (g *Ingester) handleIngestPDF(path string, logger *zerolog.Logger) {
	...
	if err != nil {
		...
		moved, err := g.MoveIfNewerThanDestinationInDir(path, g.TempPDF())
	...
	case "flattening":
		...
		moved, err := g.MoveIfNewerThanDestinationInDir(path, origin)
		if err != nil {
	...
	case "marking":
		...
		if g.IsSameAsSelfInDir(path, origin) {
			...
			moved, err := g.MoveIfNewerThanDestinationInDir(path, destination)
	...
	case "moderating":
		...
		if g.IsSameAsSelfInDir(path, origin) {
			...
			g.MoveIfNewerThanDestinationInDir(path, destination)
			return
		}
	case "checking":
	...
	default:
		...
		g.MoveIfNewerThanDestinationInDir(path, g.TempPDF())
		return
